...
enum class XMaterial(data: Int, @Nonnull vararg legacy: String) {
	...
	/**
	 * The data value of this material [pre-flattening](https://minecraft.gamepedia.com/Java_Edition_data_values/Pre-flattening).
	 *
	 *
	 * Can be accessed with [ItemStack.data] then `MaterialData#getData()`
	 ...
	 */
	@Nonnull
	val legacy: Array<String> = legacy.filterNotNull().toTypedArray()
	...
	init {
		...
		if ((!Data.isNewVersion && this.isDuplicated) || (Material.getMaterial(this.name).also { mat = it }) == null) {
	...
	constructor(vararg legacy: String?) : this(0,legacy.toString())
	...
	private val isPlural: Boolean
		 ...
		 * @since 8.0.0
		 */
		get() =// this.name().charAt(this.name().length() - 1) == 'S'
			this == CARROTS || this == POTATOES
	 ...
	 *
	 *
	 * Supports [String.contains] `CONTAINS:NAME` and Regular Expression `REGEX:PATTERN` formats.
	 *
	 *
	 * **Example:**
	 * <blockquote><pre>
	 * XMaterial material = [.matchXMaterial];
	 ...
	 *
	 *
	 * Want to learn RegEx? You can mess around in [RegExr](https://regexr.com/) website.
	 *
	...
	fun isOneOf(materials: Collection<String>?): Boolean {
		...
		for (comp in materials) {
			...
			if (checker.startsWith("REGEX:")) {
				...
				val pattern = CACHED_REGEX.getUnchecked(comp)
	...
	fun setType(@Nonnull item: ItemStack): ItemStack {
		...
		item.type = material!!
		if (!Data.isNewVersion && material.maxDurability <= 0) item.durability = data.toShort()
		return item
	}
	...
	override fun toString(): String {
		return WordUtils.capitalize(name.replace('_', ' ').lowercase())
	}
	...
	fun parseItem(): ItemStack? {
		...
		return if (Data.isNewVersion) ItemStack(material) else ItemStack(
			material, 1,
			data.toShort()
		)
	}
	...
	fun isSimilar(@Nonnull item: ItemStack): Boolean {
		...
		return Data.isNewVersion || item.durability == data.toShort() || item.type.maxDurability > 0
	}
	...
	private val isDuplicated: Boolean
		 ...
		 *
		 *
		 * This method should not be called if the version is after the flattening update [this.isNewVersion]
	...
	private object Data {
		 ...
		 */
		val version: Int
			get() = getMajorVersion(Bukkit.getVersion()).substring(2).toInt()
			/**
		 ...
		 * @since 3.0.0
		 */
		@get:Deprecated("Use {@code XMaterial.supports(13)} instead. This method name can be confusing.",
			ReplaceWith("Data.field", "net.crewco.PolicePlugin.Util.XMaterial.Data")
		)
	...
	companion object {
		 ...
		 * @since 3.4.0
		 */
		private val CACHED_REGEX: LoadingCache<String?, Pattern?> = CacheBuilder.newBuilder()
			.expireAfterAccess(3, TimeUnit.HOURS)
			.build(object : CacheLoader<String?, Pattern?>() {
				override fun load(@Nonnull str: String?): Pattern? {
					try {
		 ...
		 *
		 *
		 * Duplications are not useful at all in versions above the flattening update [this.isNewVersion]
		 * This set is only used for matching materials, for parsing refer to [.isDuplicated]
		 *
		 * @since 3.0.0
		 */
		private var DUPLICATED: MutableSet<String>? = null
		...
		init {
			if (Data.isNewVersion) {
				...
				(DUPLICATED as HashSet<String>).add(GRASS.name)
				(DUPLICATED as HashSet<String>).add(MELON.name)
				(DUPLICATED as HashSet<String>).add(BRICK.name)
				(DUPLICATED as HashSet<String>).add(NETHER_BRICK.name)
			}
		}

		@get:Deprecated("Use {@code !XMaterial.supports(9)} instead.",
			ReplaceWith("!supports(9)", "net.crewco.PolicePlugin.Util.XMaterial.Companion.supports")
		)
		val isOneEight: Boolean
			/**
			 * This is just an extra method that can be used for many cases.
			 * It can be used in [org.bukkit.event.player.PlayerInteractEvent]
		...
		private fun matchXMaterialWithData(@Nonnull name: String): Optional<XMaterial> {
			...
			if (index != -1) {
				...
				try {
					...
					val data = StringUtils.deleteWhitespace(name.substring(index + 1)).toInt().toByte()
		...
		fun matchXMaterial(@Nonnull item: ItemStack): XMaterial {
			...
			val data = (if (Data.isNewVersion || item.type.maxDurability > 0) 0 else item.durability).toByte()
		 ...
		 * @param data the data value of the material. Is always 0 or [.UNKNOWN_DATA_VALUE] when [this.isNewVersion]
		 *
		...
		protected fun matchDefinedXMaterial(@Nonnull name: String, data: Byte): Optional<XMaterial> {
			...
			if (Data.isNewVersion || (!isAMap && data <= 0 && !(isDuplicated(name).also { duplicated = it }))) {
			...
			if (!Data.isNewVersion && oldXMaterial.isPlural && (if (duplicated == null) isDuplicated(name) else duplicated!!)) return getIfPresent(
				name
			)
      ...
      This method obviously doesn't work for 1.13+ and will not be supported. This is only here for debugging purposes."""
		 ...
		 *
		 * @param version Supports [Bukkit.getVersion], [Bukkit.getBukkitVersion] and normal formats such as "1.14"
		 *
		 ...
		 */
		@Nonnull
		fun getMajorVersion(@Nonnull version: String): String {
			var version = version
			Validate.notEmpty(version, "Cannot get major Minecraft version from null or empty string")

			// getVersion()
			var index = version.lastIndexOf("MC:")
			if (index != -1) {
		...
		fun fromString(string: String): XMaterial {
			return matchXMaterial(string).get()
		}

		fun fromStringWithData(string: String, data: Byte): XMaterial {
			return matchDefinedXMaterial(string, data).get()
		}
	}
}