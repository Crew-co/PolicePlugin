...
class WantedGui  @Inject constructor(private val plugin:Startup){
	fun view(player: Player): Gui {
		...
		gui.create(
			gui.createTemplate("&8WantedList View", 54).fillRow(0, Material.GRAY_STAINED_GLASS_PANE, " ")
	...
	fun add(player: Player): Gui {
		val gui = Gui(plugin)
		val items: MutableList<ItemStack> = Bukkit.getOnlinePlayers().stream()
			.filter { p: Player? -> !wantedListManager.getPlayers().containsKey(p!!.uniqueId) }
			.map { p: Player? -> utilsManager.getSkull(p!!) }.toList()
		val heads = items.toTypedArray()
		gui.create(
			gui.createTemplate("&8Add a Player - Player Selection", 54)
				.fillRow(0, Material.GRAY_STAINED_GLASS_PANE, " ").fillRow(5, Material.GRAY_STAINED_GLASS_PANE, " ")
				...
				.setItem(5, 7, Material.LIME_STAINED_GLASS_PANE, "&aNext").noClick().onClick { e ->
					if (e != null) {
						if (e.slot == 52) gui.nextPage(player)
							...
							&& e.currentItem!!.type.equals(Material.PLAYER_HEAD)
						) {
							priority(
								player,
								(e.currentItem!!.itemMeta as SkullMeta).owningPlayer!!,
								0.toByte()
							)
								.show(player, 0)
						}
		...
		gui.getPage(0)?.onClick { e ->
			if (e != null) {
				if (e.slot == 52) gui.nextPage(player)
					...
					view(player).show(player, 0)
				} else if (e.currentItem != null && e.currentItem!!.type.equals(Material.PLAYER_HEAD)) {
					priority(
						player,
						(e.currentItem!!.itemMeta as SkullMeta).owningPlayer!!,
						0.toByte()
				...
				if (e != null) {
					if (e.slot == 25) {
						wantedListManager.addUpdate(
							player.uniqueId,
							WantedPlayer(target.uniqueId, player.uniqueId, prior[0], "")
						)
						player.sendMessage(
						...
						if (returnTo == 0.toByte()) {
							add(player).show(player, 0)
						} else wantedListManager.getPlayers().get(target.uniqueId)
							?.let { update(player, it).show(player, 0) }
					} else if (e.slot > 10 && e.slot < 16) {
	...
	fun confirm(user: Player, wp: WantedPlayer, confirm: Byte): Gui {
		...
		skullMeta.setDisplayName(
			wantedListManager.getTemplate()?.get(0)?.let {
				utilsManager.color(
					(if (p.player == null) p.name else p.player!!.displayName)?.let { it1 ->
						it.replace(
							"%player%",
							it1
						)
		...
		skullMeta.lore = wantedListManager.getTemplate()?.subList(1, wantedListManager.getTemplate()!!.size)?.let {
			utilsManager.colorList(
				it,
